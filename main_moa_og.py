...
def main():
    ...
    # --- 1) Define helper to load a fresh model per checkpoint ---
    def load_expert_from_checkpoint(chkp_path, model_class=None, config_path=None):
        """
        Load a fresh model instance for each checkpoint path.
        - model_class: optional class (e.g. GitForCausalLM) to instantiate; if None we use AutoModel.
        - config_path: path to config / pretrained identifier if required.
        """
        if model_class is None:
            # fallback to AutoModel (adjust if your experts are causal LM wrappers)
            model = AutoModel.from_pretrained(config_path if config_path is not None else chkp_path)
            model.load_state_dict(torch.load(chkp_path, map_location="cpu"), strict=False)
        else:
            # if you use a custom model class (e.g. GitForCausalLM)
            cfg = AutoConfig.from_pretrained(config_path if config_path is not None else chkp_path)
            model = model_class(cfg)
            sd = torch.load(chkp_path, map_location="cpu")
            model.load_state_dict(sd, strict=False)
        model.to(device)
        model.eval()
        return model
    
    # --- 2) Wrap experts to produce a pooled vector output (uniform interface) ---
    class ExpertWrapper(nn.Module):
        def __init__(self, expert_model, pool_method="mean"):
            super().__init__()
        ...
        def forward(self, batch):
            """
            batch: a dict or tensor that the underlying expert accepts.
            Return: pooled vector [batch, hidden_dim]
            IMPORTANT: adapt how you call the expert to match its expected inputs.
            """
            # Example assumptions:
            # - If batch is dict with 'pixel_values' or 'input_ids', forward appropriately.
            # - If the expert returns a ModelOutput/dict, attempt to get last_hidden_state.
            # - If expert outputs sequence [b, seq, d] -> perform pooling to [b, d].
            if isinstance(batch, dict):
                # adapt these keys to what your expert expects (pixel_values, input_ids, attention_mask, etc.)
                kwargs = {}
                ...
                # call expert - may return ModelOutput or tensor
                out = self.expert(**kwargs)
            else:
                out = self.expert(batch)
    
            # Unpack typical HF outputs
            if isinstance(out, dict) or hasattr(out, "last_hidden_state"):
                seq = getattr(out, "last_hidden_state", out.get("last_hidden_state", None))
            else:
                seq = out
    
            # If still None, try common attributes
            if seq is None:
                # try out['hidden_states'] etc. If not found, assume out is tensor
                if isinstance(out, dict) and "hidden_states" in out:
                    seq = out["hidden_states"][-1]
                else:
                    seq = out
    
            # Now pool sequence -> [batch, hidden_dim]
            if seq is None:
                raise RuntimeError("Could not extract hidden states from expert output. Adapt ExpertWrapper.")
            if seq.dim() == 3:  # [b, seq_len, dim]
                ...
                raise RuntimeError("Unexpected expert output shape: " + str(seq.shape))
    
            return pooled  # [batch, dim]
    
    # --- 3) Soft gating MoE with einsum ---
    class SoftGatingMoE(nn.Module):
        def __init__(self, expert_wrappers, hidden_dim, num_classes, freeze_experts=True, load_coef=0.01):
            super().__init__()
            self.experts = nn.ModuleList(expert_wrappers)
            self.num_experts = len(expert_wrappers)
        ...
        def forward(self, batch):
            """
            batch: dict from your dataloader (moved already to device).
            Returns logits [b, num_classes], aux_loss (scalar), and gate usage stats.
            """
            # 1) We need a gating input: compute a cheap pooled representation to feed gate.
            # Here we compute a lightweight pooled vector by calling the FIRST expert on the batch
            # but **without** disabling grads for gate — the experts may be frozen.
            # If you have a cheap encoder for gating, use that instead.
            with torch.no_grad():
                pooled0 = self.experts[0](batch)  # [b, hidden_dim]
    
            # Use pooled0 as gate input (no grad through expert). If you'd rather compute gate from inputs,
            # replace with a small encoder applied to batch raw inputs.
            gate_input = pooled0.detach()  # [b, hidden_dim]
    
            logits = self.gate(gate_input)         # [b, E]
            gates = F.softmax(logits, dim=-1)      # [b, E]
    
            # 2) compute each expert's pooled output (we call wrapped experts which return pooled vectors)
            expert_outputs = []
            for expert in self.experts:
                # If experts are frozen you can call them with torch.no_grad() to save memory.
                if all(p.requires_grad == False for p in expert.parameters()):
                    with torch.no_grad():
                        out = expert(batch)          # [b, hidden_dim]
                else:
                    out = expert(batch)
                expert_outputs.append(out)
    
            # stack -> [b, hidden_dim, E]
            expert_outputs = torch.stack(expert_outputs, dim=-1)
    
            # combine using einsum:
            # gates [b, E], expert_outputs [b, hidden, E] -> combined [b, hidden]
            combined = torch.einsum("be, bhe -> bh", gates, expert_outputs)
    
            logits_task = self.classifier(combined)   # [b, num_classes]
            ...
            # debugging info
            usage = mean_gates.detach().cpu().tolist()
    
            return logits_task, aux_loss, usage
    ...
    # Example: expert_model = load_expert_from_checkpoint(chkp, model_class=GitForCausalLM, config_path=model_dsi_path)
    expert_wrappers = []
    for chkp in checkpoint_paths:
        # NOTE: adapt model_class/config_path if needed
        try:
            # Try to load using the same class as model_dsi if available
            print("load model from ", chkp)
            expert_model = load_expert_from_checkpoint(chkp, model_class=None, config_path=model_dsi_path)
        except Exception as e:
            print(f"Warning: fallback loading {chkp} with AutoModel due to {e}")
            expert_model = load_expert_from_checkpoint(chkp, model_class=None, config_path=None)
    
        expert_wrappers.append(ExpertWrapper(expert_model, pool_method="mean"))
    ...
    moe = SoftGatingMoE(expert_wrappers, hidden_dim=hidden_dim, num_classes=num_classes, freeze_experts=True, load_coef=0.01).to(device)
